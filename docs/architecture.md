...
## Shared Core And Clients

Ankountant ships two clients on one shared Rust core: the PyQt/Svelte desktop app
and the native SwiftUI iOS app. Both use `rslib/` and the
`proto/anki/*.proto` contract. Desktop reaches Rust through `pylib/rsbridge`;
iOS reaches Rust through `ios/anki-bridge-rs` and `ios/AnkiRust.xcframework`.
...
The native iOS client lives under `ios/`. It uses Swift protobuf types generated
into `ios/Sources/AnkiProto/`, hand-maintained service/method IDs in
`ios/Sources/AnkiBackend/AnkiBackend.swift`, and a compiled copy of `rslib/`
packaged as `ios/AnkiRust.xcframework`. See `ios/CLAUDE.md` and
`ios/ARCHITECTURE.md` before changing it.
...
The Python/Rust bridge uses them to pass data back and forth, TypeScript uses
generated callers for web routes, and iOS uses generated Swift message types plus
hand-maintained service/method IDs. Field numbers and service/method order are
compatibility-sensitive because both desktop and iOS dispatch the same contract.
