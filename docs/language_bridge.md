Ankountant's codebase uses the desktop layers inherited from Anki plus a native
iOS Swift client.

1. The web frontend, created in Svelte and typescript,
2. The Python layer and
3. The core Rust layer, and
4. The iOS Swift layer under `ios/`.

Each desktop layer can make RPC (Remote Procedure Call) to the layers below it. While it should be avoided, Python can also invoke Typescript functions. The Rust layers never make calls to the other layers. iOS calls the same Rust backend contract through `ios/anki-bridge-rs` and generated Swift protobuf message types. Note that Rust can make RPC to AnkiWeb and other servers, which is out of scope of this document.
...
In this section we'll consider how to make Remote Procedure Call (RPC) from languages used in Ankountant.
...
### Invoking Method From Swift

Swift protobuf message types are generated into `ios/Sources/AnkiProto/*.pb.swift`
by `ios/scripts/generate-protos.sh`. Service and method IDs are not generated:
after any proto service/method add or reorder, run `just check`, inspect
`out/pylib/anki/_backend_generated.py`, and update
`ios/Sources/AnkiBackend/AnkiBackend.swift` by hand so Swift dispatches the same
backend indices as Python.
