...
makes a lot of notes about backwards compatibility. In Ankountant, desktop and
iOS share collections and dispatch the same protobuf RPC contract, so field
numbers and service/method order are compatibility-sensitive. Python, Rust, and
TypeScript regenerate from `proto/`, but iOS service/method IDs in
`ios/Sources/AnkiBackend/AnkiBackend.swift` are maintained by hand and must be
re-derived from `out/pylib/anki/_backend_generated.py` after service/method
changes.
