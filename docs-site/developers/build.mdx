...
- Start with the relevant `just` recipe, such as `just check`, `just build`, or
  `just run`, so dependency setup and generated inputs stay consistent.
- If you need low-level Ninja diagnostics, add or use a `just` recipe that wraps
  the diagnostic command instead of invoking the generated Ninja wrapper directly.
