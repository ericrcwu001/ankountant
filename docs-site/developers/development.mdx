...
- [just](https://just.systems/man/en/packages.html) command runner.
  Install with `brew install just` or `uv tool install just`. The repo root
  `justfile` is the command surface for building, running, testing, linting, and
  formatting this fork.
...
Use the same `just check` recipe on every platform.

You can also run specific checks. For example, if you see during the checks
that a Svelte check is failing, use `just test-ts` or `just lint` for the
...
`just` recipe before documenting a supported installer command here.
