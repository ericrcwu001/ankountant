...
Two fixes flipped it to a clean PASS: (1) generation now requires a _substantive_
...
- **REG · MCQ · IRS Pub 970 p.30** — "What is the maximum amount by which the
  ...
  (source: _"…reduce the amount of your income subject to tax by up to $2,500."_).
...
| App still scores with AI off                                     | deterministic offline embed/gen/judge backends                                              | dedicated keyless suite; `CARDGEN_OFFLINE=1`        |
...
1. **Ship rate ~54%** for these 161 cards (generated before the fix). The judge
   ...
   substantive source sentence and drops stubs, so the _next_ full run should ship
