...
| App still gives a score with AI switched off                       | Implemented | Runtime study/readiness is deterministic; card generation is build-time only; offline cardgen path has a dedicated keyless suite documented in `RAG_RUN_RESULTS.md` |
| Importable CPA bank                                                | Done        | `tmpl4` template deck ships 325 cards in `tools/cardgen/out/tmpl4/cpa_bank.apkg`; desktop "CPA Bank" loader imports available generated packs              |
| 50k load-test pack                                                 | Done        | `just cardgen-stress` emits `stress_bank_part*.apkg` shards tagged `stress` / `dup::<n>`; desktop "50k Stress Test" loader imports the duplicate pack      |

## Rust Evidence

| Requirement                                  | Status | Evidence                                                                                                                                             |
| -------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Determinism / A2 ablation / paraphrase proof | Done   | `docs_ankountant/evidence/{determinism,ablation,paraphrase}.html`, regenerated by `just ankountant-evidence`                                         |
| Undo still works; collection not corrupt     | Done   | `docs_ankountant/evidence/undo.html` covers add, answer, A2 `answer_card`, edit, bury, suspend, config, add-deck, full undo/redo, and db integrity   |
| One-command latency benchmark                | Done   | `docs_ankountant/evidence/latency.html`, regenerated by `just ankountant-bench`; committed run is the default 10k-card release engine-floor artifact |

## Mobile

| Requirement                                                   | Status                          | Evidence                                                                                                                                                                                 |
| ------------------------------------------------------------- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phone builds and runs                                         | Done                            | `xcodebuild build -project ios/AnkountantApp/AnkountantApp.xcodeproj -scheme AnkountantApp -destination 'platform=iOS Simulator,name=iPhone 17 Pro Max' -quiet` and `just test-ios` pass |
...
| User-visible sync log                                         | Implemented                     | `SyncCoordinator` records capped timestamped info/warning/error entries; `SyncSheet` renders the log and records cancel/auth/full-sync events                                            |
...
- Desktop TS/Svelte checks: `just test-ts` / `just lint` are the current command surface.
