...
Scope note: the generated Rust evidence pages are regenerated by the `just`
recipes below. The mentor-facing index pages are static summaries that point at
those generated artifacts plus the cardgen reports; update them whenever the
headline proof numbers or rubric mapping changes.
