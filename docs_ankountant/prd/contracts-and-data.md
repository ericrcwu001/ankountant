...
Append methods only — **never reorder existing methods** (protects the hand-maintained iOS indices). Exam date has its own scheduler RPCs because it is persisted as a sync-safe settings note, not as a `col` config blob.

```proto
message ComputeExamScheduleRequest { string section = 1; string exam_date = 2; } // ISO-8601 preview override
...
message Readiness {
  ...
  double point_estimate = 6; double coverage = 7; int64 generated_at = 8;
...
message SetExamDateRequest { string section = 1; string date = 2; } // empty clears
message GetExamDateRequest { string section = 1; }
message GetExamDateResponse { string date = 1; }
```

Codegen: edit `proto/anki/scheduler.proto` → **`just check`** (regenerates Rust `rslib/src/services.rs`, Python `out/pylib/anki/_backend_generated.py`, TS `out/ts/lib/generated/backend.ts`). Wire scheduler RPC entry points through `rslib/src/scheduler/service/mod.rs`; keep Ankountant-specific logic in `rslib/src/ankountant/`; surface Python via `pylib/anki/scheduler`.

**iOS index resync (FR-6) — mandatory after each proto change:** re-derive scheduler method indices from `out/pylib/anki/_backend_generated.py` and update `ios/Sources/AnkiBackend/AnkiBackend.swift` (scheduler service id 13). Swift protobuf message types are generated separately by `ios/scripts/generate-protos.sh`; method/service IDs are still hand-maintained. Add an iOS client **dispatch smoke test** that round-trips one known request/response per new method to catch mis-dispatch (silent, no compile error otherwise).
...
- `col` config JSON: `Collection::get_config_json`/`set_config_json`; namespaced `ankountant.*` keys (Reader already uses `ankountant.reader.progress`). Use only for blob-style config/cache; per-key settings that must merge across devices use Settings notes.
