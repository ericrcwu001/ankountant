...
- **Hot index.** The root `CLAUDE.md` carries a lean (~40-line) _Architecture
  Map_: pointers, not prose. It is loaded on every turn, so an agent always knows
...
- **Cold nested context.** Detailed local knowledge lives in 5 nested `CLAUDE.md`
  ...
  _cold_: auto-loaded only when an agent is actually working in that subtree, so
...
- **Cursor reconciliation.** `AGENTS.md` is symlinked to `CLAUDE.md`, so Cursor
  ...
  `.cursor/rules`. Instead, `.cursor/rules/*.mdc` are thin _routers_: glob
...
- **Ignore strategy.** Index noise (lockfiles, the docs site) goes in
  `.cursorindexingignore` — kept out of the search index but still _readable_ on
