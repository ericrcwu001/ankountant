...
public enum CardHTMLRewriter {
    ...
    private static func rewriteSoundMarkers(_ body: String) -> String {
        ...
        regex.enumerateMatches(in: body, range: NSRange(location: 0, length: ns.length)) { match, _, _ in
            ...
            let id = "ankountant-audio-\(UUID().uuidString)"
            result += """
            <button class="ankountant-play" onclick="ankountantPlay('\(id)')" aria-label="Play audio">\u{25B6}</button>\
            <audio id="\(id)" src="ankountant-asset://media/\(encoded)" preload="metadata"></audio>
            """
    ...
    /// `<img src="ankountant-asset://media/X">`. Known scope limits — acceptable for
    ...
    private static func rewrittenSrc(_ src: String) -> String {
        if src.hasPrefix("http://") || src.hasPrefix("https://") ||
           src.hasPrefix("data:") || src.hasPrefix("ankountant-asset://") {
            return src
        }
        let encoded = src.addingPercentEncoding(withAllowedCharacters: safeFilenameAllowed) ?? src
        return "ankountant-asset://media/\(encoded)"
    }
}
