// AnkountantApp/Sources/Widgets/WidgetTimelineProvider.swift
import WidgetKit
import Foundation
...
struct WidgetTimelineProvider: AppIntentTimelineProvider {
    typealias Intent = AnkountantWidgetIntent
    ...
    func snapshot(for configuration: AnkountantWidgetIntent, in context: Context) async -> WidgetEntry {
    ...
    func timeline(for configuration: AnkountantWidgetIntent, in context: Context) async -> Timeline<WidgetEntry> {
        let deckId = Int64(configuration.deck?.id ?? "0") ?? 0
