...
struct ChapterReaderView: View {
    ...
    private func requestSelectionForNote() {
        NotificationCenter.default.post(
            name: .ankountantReaderRequestSelection,
            object: nil
        )
    }
    ...
    private func wrappedHTML(_ content: String) -> String {
        ...
        \(pageBreakRule)
        \(rubyRule)
        ::highlight(ankountant-reader-selection) {
...
extension Notification.Name {
    ...
    static let ankountantReaderRequestSelection = Notification.Name("ankountantReaderRequestSelection")
}
...
private struct ChapterWebView: UIViewRepresentable {
    ...
    /// "make note" button fires the `.ankountantReaderRequestSelection`
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        if onTapLookup != nil {
            userContent.add(context.coordinator, name: "ankountantLookup")
    ...
    private var tapScript: String {
        ...
        document.addEventListener('click', function(e) {
          ...
          if (phrase.length > 0) {
            window.webkit.messageHandlers.ankountantLookup.postMessage(phrase);
          }
        }, true);
        """
    }
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, UIScrollViewDelegate, WKScriptMessageHandler {
        ...
        func attach(webView: WKWebView) {
            ...
            selectionObserver = NotificationCenter.default.addObserver(
                forName: .ankountantReaderRequestSelection,
                object: nil,
                queue: .main
            ) { [weak self] _ in
                Task { @MainActor in
        ...
        func userContentController(
            ...
            guard message.name == "ankountantLookup",
