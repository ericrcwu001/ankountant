...
struct DocReviewTaskView: View {
    ...
    private func submit() async {
        ...
        do {
            ...
            let submitDocReview = performanceClient.submitDocReview
            ...
            let resp = try await Task.detached(priority: .userInitiated) {
                try submitDocReview(noteId, submissionJson, confidenceValue, latency)
            }.value
    ...
    private func startLearningFeedbackIfNeeded(
        ...
        guard let request = buildTbsLearningFeedbackRequest(
            ...
            reveal: reveal,
            stepResults: results,
            userAnswerText: docReviewLearningFeedbackUserAnswer(model: model, blanks: blanks)
        ) else {
