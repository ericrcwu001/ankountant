...
export function readableTbsLoadError(error: unknown): string {
    const raw = error instanceof Error ? error.message : String(error);
    const trimmed = raw.trim();
    if (trimmed === "") {
        return "The TBS task could not be loaded.";
    }
    ...
    return prefix === ""
        ? "The TBS task could not be loaded."
        : `${prefix}. The TBS task could not be loaded.`;
}
